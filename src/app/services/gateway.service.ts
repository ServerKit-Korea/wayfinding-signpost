...
export class GatewayService {
  ...
  set serverIP(ip: string) {
    if (
      this._server_ip.substring(0, this._server_ip.indexOf("/") + 1) !=
      "http://"
    ) {
  ...
  get(url: string): Observable<Response | any> {
    return this.http
      .get(this._server_ip + url, { headers: this._headers })
  ...
  put(
    ...
    if (importHeader != undefined && importHeader) {
      return this.http
        .put(this._server_ip + url, body, { headers: this._headers })
        .pipe(timeout(this.timeout));
    }
    return this.http
      .put(this._server_ip + url, body)
      .pipe(timeout(this.timeout));
  }
  ...
  delete(url: string, importHeader?: boolean): Observable<Response | any> {
    if (importHeader != undefined && importHeader) {
      return this.http
        .delete(this._server_ip + url, { headers: this._headers })
        .pipe(timeout(this.timeout));
    }
